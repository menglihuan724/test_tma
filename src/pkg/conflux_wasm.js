...
/**
 * aes cbc pkcs7 加密，兼容 CryptoJS
 * @param {string} cipher
 * @param {string} key
 * @param {string} original_type
 * @returns {any}
 */
export function decrypt(cipher, key, original_type) {
    try {
        ...
        const ptr2 = passStringToWasm0(original_type, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('conflux_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
